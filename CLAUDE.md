...
- A hub-native peer (Pi, the local worker) does not get to call its own message `important`: `capPriority` caps it unless it answers an `important` envelope addressed to it. Do not bypass it by setting `priority` in the adapter.
