...
| | M5 step 5 with a real TUI (pause driven by Codex's own numbers) | not run: needs an interactive `ahub codex` session and an account near its limit; the real numbers are read through the daemon (row above) |
