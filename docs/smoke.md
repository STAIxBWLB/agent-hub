...
The new probes used an isolated temporary git project, the installed
`@staix/agent-hub 0.3.0`, Claude Code 2.1.278 and Kimi 2.0.1. Commands in that
project explicitly unset `AGENTHUB_STATE_DIR`: a shell launched inside a hub
...
| Switchyard two-target routing and escalation | pass (routing only) | Real Switchyard 0.2.0 selected GLM for the initial `sy/coding` request and DeepSeek after a synthetic critical tool-result fixture. In one `sy/review` session with `confirmations=2`, selection was GLM, GLM, then DeepSeek after two trouble-history submissions. Evidence came from `x-model-router-selected-model`; real router, classifier and targets were used. |
